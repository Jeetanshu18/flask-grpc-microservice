...
meeting_service_channel = grpc.insecure_channel('localhost:4000')
meeting_service_stub = meeting_pb2_grpc.MeetingStub(meeting_service_channel)
...
def create_user():
    try:
        ...
        meeting_data = meeting_pb2.GetMeetingByIdRequest(
            user=meeting_pb2.User(
                id=json_data['user']['id'],
                user_id=json_data['user']['user_id'],
                name=json_data['user']['name'],
                email=json_data['user']['email'],
                profile_logo=json_data['user']['profile_logo'],
                group_email=json_data['user']['group_email'],
                verified_email=json_data['user']['verified_email']
            ),
            id=json_data['id']
        )

        meeting_response = meeting_service_stub.get_meeting_by_id(meeting_data)

        return jsonify(MessageToDict(meeting_response, preserving_proto_field_name=True)), 201
